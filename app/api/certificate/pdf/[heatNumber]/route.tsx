...
// Build a module-level static Map lookup to meet O(1) single-scan performance requirements
const certificatesMap = new Map<string, RecordType>();
for (const record of demoRecords as any[]) {
  const key = record.heatNumber.trim().toLowerCase();
  if (!certificatesMap.has(key)) {
    certificatesMap.set(key, record as RecordType);
  }
}
...
export async function GET(
  request: NextRequest,
  { params }: { params: { heatNumber: string } }
) {
  const heatNumber = (params.heatNumber || "").trim().toLowerCase();
  ...
  // O(1) map search
  const certificate = certificatesMap.get(heatNumber);
  ...
  try {
    ...
    const pdfBuffer = (await pdf(doc).toBuffer()) as any;

    return new NextResponse(pdfBuffer, {
      headers: {
        "Content-Type": "application/pdf",
        "Content-Disposition": `inline; filename="MTC_${certificate.heatNumber}.pdf"`,
        "Cache-Control": "public, max-age=86400, s-maxage=86400", // cache generated PDFs for 24h
      },
    });
  } catch (error) {
    console.error("PDF generation failed:", error);
    return new NextResponse("Internal Server Error during PDF generation", { status: 500 });
  }
}
